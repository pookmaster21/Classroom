...
def checkifwon(player):
	...
	# checking rows
	for i in range(3):
		if b_list[i][0]["text"] == player and b_list[i][1]["text"] == player and b_list[i][2]["text"] == player:
			for g in range(3):
	...
	# checking columns
	for i in range(3):
		if b_list[0][i]["text"] == player and b_list[1][i]["text"] == player and b_list[2][i]["text"] == player:
			for g in range(3):
	...
	# checking diagonals
	if b_list[0][0]["text"] == player and b_list[1][1]["text"] == player and b_list[2][2]["text"] == player:
		winner = True
		messagebox.showinfo("Tic Tac Toe", f"CONGRATULATIONS! {player} Wins!!")
		disable_all_buttons()
