...
public class test {
    ...
    public static int countTav(BinNode<Pair> bt, char tav) {
        if (bt == null) {
            return 0;
        }
        if (bt.getValue().getTav() == tav) {
            return 1 + countTav(bt.getLeft(), tav) + countTav(bt.getRight(), tav);
        }
        return countTav(bt.getLeft(), tav) + countTav(bt.getRight(), tav);
    }

    public static boolean isTavBigger(BinNode<Pair> bt, char tav) {
        if (bt == null || bt.isLeaf()) {
            return true;
        }
        if ((bt.hasLeft() && bt.getLeft().getValue().getTav() > tav) || (bt.hasRight() && bt.getRight().getValue().getTav() > tav)) {
            return false;
        }
        if (bt.hasLeft() && bt.hasRight()) {
            return isTavBigger(bt.getLeft(), bt.getLeft().getValue().getTav()) && isTavBigger(bt.getRight(), bt.getRight().getValue().getTav());
        } else if (bt.hasLeft()) {
            return isTavBigger(bt.getLeft(), bt.getLeft().getValue().getTav());
        }
        return isTavBigger(bt.getRight(), bt.getRight().getValue().getTav());
    }

    public static boolean isTavVeod(BinNode<Pair> bt) {
        if (bt == null) {
            return true;
        }
        return isTavBigger(bt, bt.getValue().getTav()) && bt.getValue().getNum() == countTav(bt, bt.getValue().getTav()) - 1;
    }

    public static void main(String[] args) {
        BinNode<Pair> bt8 = new BinNode<>(null, new Pair(0, 'e'), null);
        BinNode<Pair> bt7 = new BinNode<>(null, new Pair(0, 'k'), bt8);
        BinNode<Pair> bt1 = new BinNode<>(new Pair(0, 'b'));
        BinNode<Pair> bt2 = new BinNode<>(bt1, new Pair(1, 'k'), bt7);
        BinNode<Pair> bt3 = new BinNode<>(new Pair(0, 'c'));
        BinNode<Pair> bt4 = new BinNode<>(null, new Pair(1, 'c'), bt3);
        BinNode<Pair> bt6 = new BinNode<>(bt4, new Pair(0, 'k'), null);
        BinNode<Pair> bt = new BinNode<>(bt2, new Pair(3, 'k'), bt6);

        System.out.println(isTavVeod(bt));
    }
}
